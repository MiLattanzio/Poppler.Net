...
- Added immutable `Document.OptionalContentGroups` and
  `DefaultOptionalContentConfiguration` metadata, including stable
  document-local identifiers, names, intents, default visibility, lock state,
  View usage, base state and radio-button groups.
- Added bounded evaluation of Optional Content Membership Dictionaries with
  `AnyOn`, `AllOn`, `AnyOff` and `AllOff` policies plus recursive `/VE`
  `And`, `Or` and `Not` visibility expressions.
- Applied optional-content visibility to marked page text and graphics, Form
  and Image XObjects, nested resources, annotations, widgets and appearance
  streams.
- Added snapshotted per-render overrides through
  `RasterRenderOptions.OptionalContentVisibility` and
  `SvgRenderOptions.OptionalContentVisibility`; unknown group identifiers are
  rejected instead of being silently ignored.
- Added the CLI `layers` command and repeatable `--layer ID=on|off` overrides
  for PNG and SVG rendering.
- Added three optional-content resource limits and cycle-safe conservative
  handling of malformed membership references.
- Added a deterministic four-page OCG/OCMD corpus and twelve metadata, text,
  graphics, annotation, form, raster, SVG, concurrency, limit, option and
  corpus-integrity regressions, bringing the suite to 183 tests.
