...
- Immutable Optional Content Group metadata and default configuration state:
  identifiers, names, intents, base state, explicit ON/OFF sets, View usage,
  locked groups and radio-button group membership.
- Marked-content visibility through `BMC`, `BDC` and `EMC`, including
  resource `/Properties` lookup and nested visibility.
- Optional-content filtering for Form and Image XObjects, annotation and
  widget dictionaries, nested Form resources and appearance streams.
- Optional Content Membership Dictionary policies `AnyOn`, `AllOn`, `AnyOff`
  and `AllOff`, plus bounded `/VE` expressions using `And`, `Or` and `Not`.
- Snapshotted per-render layer overrides for managed PNG and SVG output plus
  the CLI `layers` command and repeatable `--layer ID=on|off`.
...
- Optional content uses the default `/D` configuration and View event.
  ...
  configuration editing are not modeled. Locked and radio-button metadata is
  exposed but does not restrict explicit programmatic render overrides.
...
expression nodes, 10,000 pages, tree depth 128 and object recursion 64. Use
