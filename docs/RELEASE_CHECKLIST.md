...
- [ ] Issue #22 is complete and the rc.1 milestone has no open release blocker.
- [ ] The approved master commit and successful rc.1 release run are recorded.
- [ ] The version-normalized callable API fingerprint remains
  `ce87b22579e9458c3c1dcdb1aa01790f15d13006ad177ad4815f1e974e63b527`,
  matching `docs/API_FREEZE.md` and the regression test.
- [ ] Any post-RC blocker fix is documented and has passed focused and full
  regression tests.

## Stable version and documentation

- [ ] Set the library, CLI, package-smoke fallback and `Document.PortVersion`
...
- [ ] Audit `docs/API.md`, `docs/COMPATIBILITY.md`, `LICENSE`, `NOTICE.md`,
  package license/repository metadata and the explicit package allowlist.
- [ ] Confirm release notes contain no prerelease label and install with
  `<PackageReference Include="Poppler.Net" Version="0.12.0" />`.

## Qualification

- [ ] Restore with the repository `NuGet.Config`, build Release with warnings
  as errors and run the complete NUnitLite suite.
- [ ] Run the managed-only verifier and the optional Poppler differential
...
- [ ] The tag guard accepts `v0.12.0` and rejects a mismatched tag.
